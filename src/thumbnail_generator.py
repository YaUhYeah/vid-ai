...
class ThumbnailGenerator:
    ...
    def _add_text_overlays(self, image: Image, text: Dict[str, str]) -> Image:
        ...
        if 'title' in text:
            ...
            draw.text((x + shadow_offset, y + shadow_offset), 
                     title_text, font=title_font, fill=(0, 0, 0, 180))
        ...
        if 'subtitle' in text:
            ...
            shadow_offset = 2
            draw.text((x + shadow_offset, y + shadow_offset),
                     subtitle_text, font=subtitle_font, fill=(0, 0, 0, 180))
    ...
    def extract_best_frame(self, video_path: str, num_samples: int = 10) -> np.ndarray:
        ...
        for i in range(num_samples):
            ...
            # Calculate frame score based on various metrics
    ...
    def _calculate_frame_score(self, frame: np.ndarray) -> float:
        ...
        # Calculate metrics
        contrast = np.std(gray)  # Image contrast
        brightness = np.mean(gray)  # Average brightness
        blur = cv2.Laplacian(gray, cv2.CV_64F).var()  # Blur detection
        
        # Detect faces (good thumbnails often have faces)
        face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        faces = face_cascade.detectMultiScale(gray, 1.3, 5)
        face_score = len(faces) * 50  # Bonus points for faces
        
        # Combine metrics into final score
        score = (contrast * 0.5 +  # Weight contrast heavily
                min(brightness, 255-brightness) * 0.3 +  # Prefer balanced brightness
                min(blur, 1000) * 0.2 +  # Some blur is OK
                face_score)  # Bonus for faces
